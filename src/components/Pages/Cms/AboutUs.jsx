...
const App = () => {
    ...
    const postData = async()=>{
        try{
            const response= await axios.post(`${import.meta.env.VITE_API_URL}about-us`,{content})
    ...
    const getData=async()=>{
        try{
            const response= await axios.get(`${import.meta.env.VITE_API_URL}about-us`)
            if(response.data.status==200){
                ...
                <Box component="form" onSubmit={submitForm }>
                    <Paper elevation={3} sx={{
                        ...
                        <Typography variant='h6' sx={{
                            textAlign: 'center'
                        }} component='h2'>About Us</Typography>
                    <CKEditor
